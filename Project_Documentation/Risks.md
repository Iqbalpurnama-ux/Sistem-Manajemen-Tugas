# Risk Analysis & Mitigation (Project OUTBREAK)
...
| **UX (Operasional)** | Email *reminder* yang dikirim masuk ke *Spam Folder* klien (Gmail/Yahoo), menyebabkan *user* komplain gagal diingatkan. | Sedang | Sedang | Menyiapkan domain kustom (contoh: `outbreak.com`) dan melakukan otentikasi domain tingkat tinggi di Resend (menambahkan *records* `DKIM`, `SPF`, dan `DMARC` pada penyedia DNS, misalnya Cloudflare) sebelum aplikasi digunakan untuk produksi ril. |
